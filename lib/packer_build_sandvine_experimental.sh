...
#                --packer-max-tries=1 --packer-to-openstack --os-project=svauto $DRY_RUN_OPT
	...
	./svauto.sh --packer-builder --base-os=centos7 --release=dev --product=svtse --version=$TSE_EXPERIMENTAL_VERSION --product-variant=vpl-test-1 --qcow2 --ova --vhd --vm-xml --sha256sum \
		--ansible-playbook-builder=svtse-servers,cloud-init,bootstrap:base_os_upgrade=yes:sandvine_main_yum_repo=yes:packages_server=$SVAUTO_MAIN_HOST:release_code_name=$RELEASE_CODE_NAME,grub-conf,udev-rules,nginx,svtse:svtse_version=$TSE_EXPERIMENTAL_VERSION:static_packages_server=$STATIC_PACKAGES_SERVER:versioned_repo=true,vmware-tools,post-cleanup-image \
		--packer-max-tries=1 --packer-to-openstack --os-project=svauto $DRY_RUN_OPT
	...
	./svauto.sh --packer-builder --base-os=centos7 --release=dev --product=svtcpa --version=$TCPA_VERSION --product-variant=vpl-test-1 --qcow2 --ova --vhd --vm-xml --sha256sum \
		--ansible-playbook-builder=svtcpa-servers,cloud-init,bootstrap:base_os_upgrade=yes:sandvine_main_yum_repo=yes:packages_server=$SVAUTO_MAIN_HOST:release_code_name=$RELEASE_CODE_NAME,grub-conf,udev-rules,nginx,svtcpa:svtcpa_version=$TCPA_VERSION:static_packages_server=$STATIC_PACKAGES_SERVER:versioned_repo=true,vmware-tools,post-cleanup-image \
		--packer-max-tries=1 --packer-to-openstack --os-project=svauto $DRY_RUN_OPT
...
#                --ansible-playbook-builder=svnda-servers,cloud-init,bootstrap:base_os_upgrade=yes,grub-conf,udev-rules,nginx,postgresql,svnda:svnda_version=$NDA_VERSION:static_packages_server=$STATIC_PACKAGES_SERVER:versioned_repo=true,vmware-tools,post-cleanup-image \
#                --packer-max-tries=1 --packer-to-openstack --os-project=svauto $DRY_RUN_OPT
