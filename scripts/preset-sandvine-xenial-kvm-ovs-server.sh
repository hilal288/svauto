...
./svauto.sh --cpu-check --hostname-check \
	...
	--ansible-inventory-builder="all,localhost,ansible_connection=local,regular_system_user=$LINUX_USER,ubuntu_install=server,os_release=ocata,openvswitch_mode=regular" \
