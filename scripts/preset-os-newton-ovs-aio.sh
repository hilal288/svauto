...
./svauto.sh --cpu-check --hostname-check \
	...
	--ansible-inventory-builder="os_aio,localhost,ansible_connection=local,regular_system_user=$UBUNTU_USERNAME,os_release=newton,ubuntu_primary_interface=$UBUNTU_PRIMARY_INTERFACE,os_dns_domain=$YOUR_DOMAIN,os_public_addr=$UBUNTU_HOSTNAME.$YOUR_DOMAIN,os_admin_addr=$UBUNTU_HOSTNAME.$YOUR_DOMAIN,deployment_mode=yes,os_mgmt=$UBUNTU_PRIMARY_INTERFACE" \
	--ansible-playbook-builder="localhost,bootstrap;base_os_upgrade=yes,grub-conf,openvswitch;openvswitch_mode=regular,openvswitch_bridges;os_neutron_ovs_enabled=yes,ubuntu-network-setup;ubuntu_primary_interface_via_ifupdown=yes;ubuntu_setup_dummy_nics=yes;ubuntu_enable_ip_masquerade=yes,os_clients,ssh_keypair,os_mysql,os_rabbitmq,os_memcached,apache2,os_keystone,os_glance,hyper_kvm,os_nova;os_nova_ctrl=yes;os_nova_cmpt=yes,os_keypair,os_nova_flavors,os_neutron;os_aio=yes;os_neutron_ctrl=yes;os_neutron_net=yes;os_neutron_ovs_enabled=yes;neutron_interface_driver=openvswitch;os_external=dummy0;os_data=dummy1,os_ext_net,os_horizon,os_heat,download-images;download_group=generic,os_glance_images,post-cleanup"
